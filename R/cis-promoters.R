...
#' Get candidates for promtoers that are regulated by methylation in cis
#' 
#' @param meth_mat Matrix with methylation values. Each row is a promoter and each column is a sample. Rownames should contain the name of the promoter (gene name).
...
#' @param min_samples minimal number of samples per gene (both expression and methylation)
#' 
...
#'  \item{fdr}{false discovery rate for detecting the cis-raget with rank value <= \code{rank}}
#' }
#' 
#' 
#' @export
cis_em_promoters <- function(meth_mat, expr_mat, promoter_intervs, k_locus_rank = 2, min_samples=100) {
    ...
    m <- tgs_cor(t(meth_mat[genes, samples]), t(expr_mat[genes, samples]), spearman=TRUE, pairwise.complete.obs=TRUE)
    ...
    cands <- enframe(diag(m), value = "cor") %>%
        left_join(enframe(diag(m_rank), value = "r")) 
